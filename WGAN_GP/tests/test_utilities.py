...
class test_utilities(unittest.TestCase):
    ...
    def test_truncation_sample(self):
    # Test the truncation sample
        self.assertEqual(tuple(get_truncated_noise(n_samples=10, z_dim=5, truncation=0.7).shape),(10, 5))
        simple_noise = get_truncated_noise(n_samples=1000, z_dim=10, truncation=0.2)
        self.assertGreater(simple_noise.max(), 0.199)
        self.assertLess(simple_noise.max(), 0.2)
        #self.assertLessA(simple_noise.max() > 0.199 and simple_noise.max(), 2)
        #self.assertGreater(simple_noise.min() < -0.199 and simple_noise.min(), -0.2)
        #self.assertGreater(simple_noise.std(), 0.117)
        #self.assertGreater(simple_noise.std(), 0.113)
